...
def test_create_insecure_clipboard():
    response = client.post("/create_clipboard/test",
                           json={"contents": "supplied contents"})    
    assert response.status_code == 201


def test_create_secure_clipboard():
    response = client.post("/create_clipboard/test_secure",
                           json={"contents": "supplied contents",
...
def test_get_real_secure_clipboard():
    response = client.post("/get_clipboard/test_secure",
...
def test_fail_get_fake_secure_clipboard():
    response = client.post("/get_clipboard/test_noexist",
                          json={"passphrase": "wrong_secret"}
                          )
    assert response.status_code == 404

def test_change_insecure_clipboard():
   response = client.post("/set_clipboard/test",
...
def test_change_secure_clipboard():
    response = client.post("/set_clipboard/test_secure",
                           json={"contents": "new_content",
                                 "passphrase": "super_secret"}
                           )
    assert response.status_code == 200
...
def test_kill_insecure_clipboard():
    response = client.post("/kill_clipboard/test",
                           json={"passphrase": ""})
    assert response.status_code == 200
...
def test_fail_kill_secure_clipboard():
    response = client.post("/kill_clipboard/test_secure",
                           json={"passphrase": ""})
    assert response.status_code == 403
